...
def get_change(year='2020'):
	...
	for day  in tqdm(cal_date):
		...
		sql="SELECT ts_code,change FROM daily where trade_date=? "
		df=pd.read_sql(sql,conn,params=(day,))
...
for y in  range(1991,2020):
	...
	if not is_exists_table(name=table_name1):
		df_vol=get_vol(year)
		df_vol.to_sql(table_name1,conn,if_exists='replace')
		print(df_vol)
	else:
		print(table_name1,len(get_columns(table_name1)))
		
	if not is_exists_table(name=table_name2):
		df_change=get_change(year)
		df_change.to_sql(table_name2,conn,if_exists='replace')
		print(df_change)
	else:
		print(table_name2,len(get_columns(table_name2)))
		
	print('用时：',int(time.time()-begin_time))
