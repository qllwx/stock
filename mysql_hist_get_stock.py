...
def get_cal_date(year='2020'):
	...
	cal_date=list(df[df.is_open==1]['cal_date'])
	return cal_date
def   column_to_df(year,col):
	cal_date=get_cal_date(year=year)
	df_result=pd.DataFrame()
	list_day=['ts_code']
	print('get_%s_%s'%(year,col))
	for day  in tqdm(cal_date):
		sql="SELECT ts_code,%s FROM daily where trade_date='%s';"%(col,day)
		df=pd.read_sql(sql,conn)
...
def get_continue(df,days=3,end_date=today):
	...
	for col in trange(days,len(df.columns)):
		f=df.iloc[-1,col]
		isget=True
		for row in range(Move_forward+days,-Move_forward):
			#print(end_date,row,col,Move_forward-days,Move_forward)
			tmp=df.iloc[row,col]
...
def ljqs_code(date=today,days=3,year='2020'):
	...
	jj=list_jj(get_continue(df_vol,days=days,end_date=date),get_continue(df_ope,days=days,end_date=date))
	return jj
	...
	for d in cal_date:
		...
		for c in code_list:
			...
			if result>0:
				rr=cursor.fetchall()
				r=rr[0][0]
				sql="insert into ljqs(trade_date,ts_code) values('%s',%s);"%(d,r)
				print(sql)
				cursor.execute(sql)
